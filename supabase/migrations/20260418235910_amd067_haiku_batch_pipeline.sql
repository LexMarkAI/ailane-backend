-- =============================================================================
-- AMD-067: Haiku Batch Pipeline + Opus Cross-Model QA
-- =============================================================================
-- Net-new objects for the Haiku Batch API extraction pipeline with Opus 4.7
-- cross-model QA sampling. Depends on the AMD-066 backfill migration
-- (20260418235909_backfill_amd066_enrichment_pipeline.sql) — do not run this
-- migration without that one.
--
-- Governance reference: AILANE-CC-BRIEF-HAIKU-BATCH-001 Addendum A §A.2.
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- A.2.3  CHECK on tribunal_enrichment.extraction_model
-- -----------------------------------------------------------------------------
-- Live DB currently has the column all-NULL, so no data cleanup is required
-- before attaching the constraint. The OLD extraction_method column keeps its
-- own pre-existing CHECK (pdf_parse|html_parse|llm_extract|manual) — that is
-- out of scope here.
DO $$
BEGIN
  IF NOT EXISTS (
